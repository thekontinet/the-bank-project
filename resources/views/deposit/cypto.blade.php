                    ...
                    <div class="col-span-full">
                        <div class="w-full">
                            ...
                            <div class="space-y-8 divide-y divide-muted-200 dark:divide-muted-800">
                                    ...
                                    <div class="relative group">
                                        <select name="account"
                                            ...
                                            @foreach ($accounts as $account)
                                                <option value="{{ $account->number }}">{{ ucfirst($account->number) }} -
                                                    {{ money($account->balance, $account->currency) }}</option>
                                            @endforeach
                                        </select>
                                        <div
                                ...
                                <!--Payment Info-->
                                <div class="flex flex-col items-center justify-center px-4 py-2 my-6 mt-8 border rounded-lg"
                                    x-show='wallet'>
                                    <img class="object-cover w-12 h-12 rounded-full" :src="wallet.image_url"
                                        :alt="wallet.name" />
                                    <h4 class="text-base" x-text="wallet.name"></h4>
                                    <div class="flex items-center w-full max-w-md mt-5 gap-x-2">
                                        <input type="text" x-bind:value="wallet.address" readonly
                                        ...
                                        <button type="button"
                                            @click="navigator.clipboard.writeText(wallet.address); alert('copied')"
                                            class="rounded-md p-1.5 text-gray-700 bg-white border border-gray-200 dark:bg-gray-900 dark:text-gray-300 dark:border-gray-700 focus:border-blue-400 focus:ring-blue-300 focus:ring-opacity-40 dark:focus:border-blue-300 focus:outline-none focus:ring transition-colors duration-300 hover:text-blue-500 dark:hover:text-blue-500">
                                            <i class="w-5 h-5 iconify" data-icon="lucide:copy"></i>
                                        </button>
                                    </div>
                                    <p
                                        class="max-w-md p-3 mx-auto mt-4 text-center text-green-500 bg-green-100 rounded-md">
                                        Copy and make deposit to the above wallet address. Funds are automatically
                                        credited immediately after transaction is verified by the blockchain.
                                    </p>
                                </div>

                                <!--Buttons-->
                                <div class="flex gap-4 py-8">
                                    <x-primary-button x-bind:disabled="!wallet">I have made
                                        payment</x-primary-button>
                                </div>
                            </div>
                        </div>
                    </div>
