<x-admin-layout>
    <header class="flex items-center justify-between mt-10 dark:text-muted-100">
        <h4 class="text-3xl font-medium uppercase">Deposit Crypto Wallet</h4>
        <x-primary-button href="{{route('admin.wallets.create')}}">Add New Wallet</x-primary-button>
    </header>

    <div class="my-6">
        <ul class="flex flex-col gap-3">
            @foreach ($wallets as $wallet)
                <li class="flex items-center gap-2 overflow-hidden border rounded-md dark:bg-muted-1000 dark:text-muted-100">
